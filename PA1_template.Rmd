...
The 5-minute interval that contains the maximum number of steps (on average across all the days in the dataset) is `r max_interval` with `r max_intervav` steps.
...
In the dataset there are `r dim(missing_data)[1]` values with missing data. The missing values will be replaced by the respective number of steps correspond to the mean total pattern previously calculated.
